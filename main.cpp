...
int main() {
    ...
    if (rez == 1) {
        ...
        cout << "Failas sugeneruotas per " << std::chrono::duration<double>(t1 - t0).count() << " s.\n";
        return 0;
    } else if (rez == 3) {
        ...
        for (int i = 0; i < kiek; ++i) {
            ...
            std::cout << "\nStudentas #" << i+1 << "\nVardas: "; cin >> v;
            std::cout << "Pavardė: "; cin >> p;
            std::cout << "Įveskite 5 ND: "; for (int j=0;j<5;++j) cin >> nd[j];
            std::cout << "Egzaminas: "; cin >> egz;
            Studentas s; s.setVardas(v); s.setPavarde(p); s.setNd(nd); s.setEgzaminas(egz);
            s.perskaiciuoti(vidurkis);
            std::cout << "Objekto adresas: " << &gr.emplace_back(s) << "\n";
        }
        ofstream out("ivedimas_rankinis.txt");
        out << left << std::setw(12) << "Vardas" << std::setw(12) << "Pavarde" << std::setw(10) << "Galutinis" << "\n";
