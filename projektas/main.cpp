...
int main() {
    ...
    if (rezimas == 1) {
        ...
        if (kon == 1) {
            ...
            for (int i = 0; i < kiek; i++) {
                ...
                cout << "Įveskite 5 namų darbų pažymius (nuo 1 iki 10): ";
                for (int j = 0; j < 5; j++) {
                    int paz;
                    cin >> paz;
                ...
                grupe.push_back(s);
                cout << "Adresas atmintyje: " << &grupe.back() << "\n";
            }
            ...
            cout << "\n Rezultatai įrašyti į failą: ivedimas_vector.txt\n";
        }
            ...
            cout << "\n Rezultatai įrašyti į failą: ivedimas_list.txt\n";
        }
    ...
    if (kon == 1) {
        ...
        << std::chrono::duration<double>(end_split - start_split).count()
        << " s\n";
        
        auto start_write = std::chrono::high_resolution_clock::now();
        ofstream out("rezultatai_vector.txt");
        if (!out) cerr << "Nepavyko sukurti rezultatu failo.\n";
        else {
            out << left << setw(12) << "Vardas"
            << setw(12) << "Pavarde"
            << setw(10) << "Egzaminas";
            if (tipas == 1)
                out << setw(15) << "Galutinis(vid)";
            else if (tipas == 2)
                out << setw(15) << "Galutinis(med)";
            else
                out << setw(15) << "Galutinis(vid)"
            ...
            for (auto &s : grupe) {
                double galVid = skaiciuotiGalutiniPagalTipa(s.pazymiai, s.egzaminas, 1);
                double galMed = skaiciuotiGalutiniPagalTipa(s.pazymiai, s.egzaminas, 2);
                out << left << setw(12) << s.vardas
                << setw(12) << s.pavarde
                << setw(10) << s.egzaminas;
                if (tipas == 1)
                    ...
                    out << setw(15) << galVid << setw(15) << galMed;
                out << "\n";
            }
        }
        auto end_write = std::chrono::high_resolution_clock::now();
        cout << "Įrašymas į failą (rezultatai_vector.txt): "
        ...
        auto start_write = std::chrono::high_resolution_clock::now();
        ofstream out("rezultatai_list.txt");
        if (!out) cerr << "Nepavyko sukurti rezultatu failo.\n";
        else {
            out << left << setw(12) << "Vardas"
            << setw(12) << "Pavarde"
        ...
        auto end_write = std::chrono::high_resolution_clock::now();
        cout << "Įrašymas į failą (rezultatai_list.txt): "
        << std::chrono::duration<double>(end_write - start_write).count()
        << " s\n";
    }
